...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = (from e in emps
                           where depts.Where(d => d.Loc == "CHICAGO")
                                      .Select(d => d.DeptNo)
                                      .Contains(e.DeptNo)
                           select e).ToList();
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.SelectMany(e => new[] { new { e.EName, e.Comm } })
                                         .Where(e => e.Comm != null)
                                         .ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e => grades, (e, s) => new { e, s })
                                                   .Where(x => x.e.Sal >= x.s.Losal && x.e.Sal <= x.s.Hisal)
                                                   .Select(x => new { x.e.EName, x.s.Grade });
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.Select(e => new { e.DeptNo, e.Sal })
                                                    .GroupBy(e => e.DeptNo)
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = from e in emps
                     where e.Sal > (from e2 in emps
                                    where e2.DeptNo == e.DeptNo
                                    select e2.Sal).Average()
                     select e.EName;
